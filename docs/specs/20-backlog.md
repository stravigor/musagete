...
| 3 | [003](./slices/003-reader-editorial-layout.md) | Reader — editorial layout, light/dark, density, accent     | drafted  | 002        | No-JS read path; tokens per ADR-0003; `@strav/view`. **Inherits from slice 001:** owns `resources/css/tokens.css`. (Auth view originally deferred here too; that deferral was rescinded — auth view + AuthForm island shipped under slice 002 Build-T2 with ad-hoc CSS Modules. Slice 003 themes the auth view + workspace forms + wizard once tokens land.) |
