# ADR-0007 — Versioning: immutable revisions with `doc.current_revision_id` pointer
...
Revisions are **immutable**. Every save creates a new `revision` row; `doc.current_revision_id` is a foreign key to the latest *published* revision; PR-style "changes" reference a `source_revision_id` (the new content) and a `base_revision_id` (the revision they branched from). On merge, `doc.current_revision_id` updates to `source_revision_id`. Diffs are computed at read time via `diff-match-patch` for word-level granularity; nothing about the diff is stored.
...
- Embeddings in the `embedding` table are keyed by `revision_id` — they never go stale because revisions never mutate.
...
### Option A — Mutable `doc.content` with a `revision_history` audit table
...
- [ ] No UPDATE statement on `revision.content` anywhere in `app/`.
- [ ] `doc.current_revision_id` foreign keys to `revision.id` with `ON DELETE RESTRICT`.
...
### 2026-05-09 — Singularize table-name references (project-wide convention)
Section: `Title`, `Context`, `Decision`, `Tradeoffs`, `Alternatives considered`, `Verification hooks`
From:    Body referenced tables in plural (`docs`, `revisions`, `embeddings`); column references read `docs.current_revision_id` / `revisions.content` / `revisions.id`.
To:      Body uses singular table names (`doc`, `revision`, `embedding`); column references read `doc.current_revision_id` / `revision.content` / `revision.id`. Decision-block phrasing kept (`'draft'`, `current_revision_id`) — only table-shaped tokens changed.
Why:     Strav uses singular table names by convention; slice 001's Build-T1 surfaced this and amended its Tech Spec. Slice 002's Build-T0 sweeps the same convention across the Design schema sketch and signed ADR-0007 so every spec document agrees before slice 002 authors its schemas. Doesn't change the decision; only the tokens used to refer to the schemas.
By:      Liva
