...
By:      Liva
**RESCINDED 2026-05-09 (slice 002 Build-T2):** the auth view + AuthForm island ship under slice 002 Build-T2 with ad-hoc `<style module>` CSS instead of `tokens.css` (slice 002's wizard precedent). Tokens.css remains slice 003's deliverable; slice 003 will theme the auth view + workspace forms + wizard when it lands tokens. The original deferral was the trigger for the demonstrability gap documented in `docs/notes/agon-vertical-slicing-enforcement.md` — UI is part of the slice; deferring it to a later slice violates AGON's vertical-slicing mandate. Slice 003's backlog inheritance row is updated to reflect: tokens.css only (the auth view ships under slice 002 Build-T2).
